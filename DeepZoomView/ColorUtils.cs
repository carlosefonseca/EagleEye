...
namespace ColorUtils
{
	public class ColorUtil
	{
		...
		public static String HueToName(int h)
		{
			return HueToName(h, 30);
		}
		...
		public static String HueToName(int h, int spread)
		{
			...
			switch (hue)
			{
				...
				case 90: return "Yellowish-Green";
		...
		public static Color ParseCardinalRGBColor(String name)
		{
			String[] parts = name.Split(new char[] { ' ', '#' }, StringSplitOptions.RemoveEmptyEntries);
			Byte R = Byte.Parse(parts[0]);
			Byte G = Byte.Parse(parts[1]);
			Byte B = Byte.Parse(parts[2]);
			return Color.FromArgb((byte)150, R, G, B);
		}
	}
	...
	public struct HslColor
	{
		...
		public string Name
		{
			get
			{
				if (this.L == 1) return "White";
				else if (this.L == 0) return "Black";
				else
				{
					if (this.S < 0.1)
					{
					...
					{
						return ColorUtil.HueToName(this.SimpleHue());
					}
				}
			}
		}
